...
export class SocketService {
   ...
   private url = 'https://172.31.8.100:8080';
  ...
  sendCaptions(currDel, contents) {
    ...
    if (!this.transcriptLoad) { // captioner is in session
      ...
      .put(`https://172.31.8.100:8080/api/transcripts/id/${this.id}`, {captions: contents}, { headers: headers })
      .map(res => res.json());
    } else { // captioner is editing a transcript
      return this.http // send the contents to the database
      .put(`https://172.31.8.100:8080/api/transcripts/id/${this.id}`, {modCaptions: contents, rawStatus: false}, { headers: headers })
      .map(res => res.json());
    }
  }
