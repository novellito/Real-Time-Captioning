...
export class SocketService {
   ...
   private url = 'https://demo-load-balancer-1858473610.us-west-2.elb.amazonaws.com';
  ...
  sendCaptions(currDel, contents) {
    ...
    if (!this.transcriptLoad) { // captioner is in session
      ...
      .put(`https://demo-load-balancer-1858473610.us-west-2.elb.amazonaws.com/api/transcripts/id/${this.id}`, {captions: contents}, { headers: headers })
      .map(res => res.json());
    } else { // captioner is editing a transcript
      return this.http // send the contents to the database
      .put(`https://demo-load-balancer-1858473610.us-west-2.elb.amazonaws.com/api/transcripts/id/${this.id}`, {modCaptions: contents, rawStatus: false}, { headers: headers })
      .map(res => res.json());
    }
  }
