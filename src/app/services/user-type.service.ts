...
export class UserTypeService {
  ...
  loadTranscript(transcriptID) {
    ...
    return this.http.get(`https://csunrealtimecaptioning.com:8080/api/transcripts/id/${transcriptID}`, {headers: headers})
    .map(res => res.json());
  }
  ...
  getTranscripts(id) {
    const headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.get(`https://csunrealtimecaptioning.com:8080/api/transcripts/courseID/${id}`, { headers: headers })
    .map(res => {
  ...
  getClasses() {
    ...
    if (JSON.parse(localStorage.getItem('user')).role === 'captioner') {
      return this.http.get(`https://csunrealtimecaptioning.com:8080/api/captionists/username/${JSON.parse(localStorage.getItem('user')).userID}`,
        {headers: headers})
        .map(res => res.json());
    } else {
      return this.http.get(`https://csunrealtimecaptioning.com:8080/api/students/username/${JSON.parse(localStorage.getItem('user')).userID}`,
      {headers: headers})
      .map(res => res.json());
    }
  }
  ...
  deleteTranscript() {
    ...
    const headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.delete(`https://csunrealtimecaptioning.com:8080/api/transcripts/id/${this.transcriptID}`, {headers: headers})
  ...
  removeClass() {
    ...
    if (this.data.role === 'student') {
      return this.http.put(`https://csunrealtimecaptioning.com:8080/api/students/username/${this.data.userName}`, {id: this.data.classID}, {headers: headers})
      .map(res => res.json()).subscribe();
    } else {
       return this.http.put(`https://csunrealtimecaptioning.com:8080/api/captionists/username/${this.data.userName}`, {id: this.data.classID},
        {headers: headers})
        .map(res => res.json()).subscribe();
    }
  }
  ...
  download(name, id, status) {
    return this.http.get(`https://csunrealtimecaptioning.com:8080/api/downloads/${id}/${status}`, {responseType: ResponseContentType.Blob})
