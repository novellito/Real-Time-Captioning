...
export class AddAClassService {
  ...
  addClassToCollection(classObj) {
    ...
    return this.http.post(`https://172.31.8.100:8080/api/classes`, data, {headers: headers})
    .map(res => res.json());
  }
  ...
  addClassToUser(data) {
    ...
    if (JSON.parse(localStorage.getItem('user')).role === 'captioner') {
      ...
      return this.http.put(`https://172.31.8.100:8080/api/captionists/username/${JSON.parse(localStorage.getItem('user')).userID}`,
      data, {headers: headers})
      .map(res => res.json());

    } else {

        return this.http.put(`https://172.31.8.100:8080/api/students/username/${JSON.parse(localStorage.getItem('user')).userID}`,
         data, {headers: headers})
        .map(res => res.json());
    }
  }
