...
export class CaptionerUtilsService {
  ...
  constructor(private http: Http) { }
   ...
   getClass(courseID) {
     ...
     return this.http.get(`http://172.31.8.18:8080/api/classes/courseID/${courseID}`, {headers: headers})
     .map(res => res.json());
    }
    ...
    createTranscript(id) {
      const headers = new Headers();
      headers.append('Content-Type', 'application/json');
      return this.http.post(`http://172.31.8.18:8080/api/transcripts`, {courseID: id} , {headers: headers})
      .map(res => res.json());
    }
    ...
    updateTranscriptTitle(id, name) {
        ...
        return this.http // send the contents to the database
          .put(`http://172.31.8.18:8080/api/transcripts/id/${id}`, {transcriptName: name}, { headers: headers })
          .map(res => res.json());
    }
